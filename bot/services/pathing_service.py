...
class PathingService:
    ...
    def find_path(self, origin: Point2, destination: Point2, consider_threats = False) -> 'path, dist':
        if consider_threats:
            pf = sc2pathlib.PathFinder(np.array(self.threat_grid, dtype=int))
        else:
            pf = sc2pathlib.PathFinder(np.array(self.pathing_grid, dtype=int))
        # print(f"Threat grid: {self.threat_grid}, pathing grid: {self.pathing_grid}")
        # print("Trying to find first path")
        # print(pf.find_path((0,0), (1,1)))
        # print(f"Second path variables {int(origin.x)}, {int(origin.y)}, {int(destination.x)}, {int(destination.y)}")
    ...
    def _test_path_find(self):
        ...
        pf = sc2pathlib.PathFinder(pathing)
